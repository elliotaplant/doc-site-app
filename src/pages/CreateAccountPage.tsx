...
const { Form } = withTypes<CreateAccountFormFields>();

export function CreateAccountPage() {
  const { loginUser, signupUser } = useIdentityContext();

  const onSubmit = async ({ email, password }: CreateAccountFormFields) => {
    try {
      ...
      if (error?.json?.error_description) {
        return { [FORM_ERROR]: error.json.error_description };
      } else {
        ...
        render={({ handleSubmit, submitError, submitting }) => (
          <form onSubmit={handleSubmit} className="flex w-full max-w-sm flex-col gap-4">
            ...
            <p className="text-text mt-6 text-center text-sm">
              ...
              <a
                className="text-primary hover:text-primary-accent font-semibold"
                href="https://docsite.io/terms-and-conditions"
              >
